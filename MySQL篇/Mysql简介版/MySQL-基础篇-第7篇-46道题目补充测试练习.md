...
- 题目和解析来源： https://www.cnblogs.com/mzhaox/p/11280234.html 
...
select a.*,s.sname
from (
    select sno,sum(score),count(cno) 
...
-- 27、1995 年出生的学生名单(注：Student 表中Sage 列的类型是int)
select sno,sname,sage,sgender from student t where YEAR(NOW())-sage = 1995
...
-- 40、统计每门课程的学生选修人数（超10 人的课程才统计）。要求输出课程号和选修人数，查询结果按人数降序排列，
--  若人数相同，按课程号升序排列
