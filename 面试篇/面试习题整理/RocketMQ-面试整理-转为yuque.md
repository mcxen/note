...
![消息队列解耦](https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/weixin-mianznxrocketmqessw-dd332b3f-d5e5-41bc-813a-9f612e582255.jpg)


*   **异步**：订单支付之后，我们要扣减库存、增加积分、发送消息等等，这样一来这个链路就长了，链路一长，响应时间就变长了。引入消息队列，除了`更新订单状态`，其它的都可以**异步**去做，这样一来就来，就能降低响应时间。

![消息队列异步](https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/weixin-mianznxrocketmqessw-40d8f782-08cf-48c4-98b2-cb125d287e93.jpg)
...
![消息队列削峰](https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/weixin-mianznxrocketmqessw-f028cb0c-b1a3-47ef-b290-f7d6f46512fb.jpg)
...
![四大消息队列对比](https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/weixin-mianznxrocketmqessw-c3493e70-67c7-4f0d-bb99-f0fe8074c807.jpg)
...
*   Pull：拉取型消费者（Pull Consumer）主动从消息服务器拉取信息，只要批量拉取到消息，用户应用就会启动消费过程，所以 Pull 称为主动消费型。（目前被废弃了）
*   Push：（默认是这个） PUSH 模式：消费端与 Broker 建立连接后，每当服务端有消息时，会将消息推送给消费端，这种模式只要消费端与服务器建立连接之后，那么这个连接就不会被断开，将持续保持着连接，此时一旦有消息出现之后， 服务器端会将新的消息通过当前的连接推送给消费端。



## 进阶

### 9.快速简答 消息可靠性，顺序性，幂等性

#### 可靠性！

在生产阶段，主要**通过请求确认机制，来保证消息的可靠传递**。存储阶段，**配置可靠性优先的 Broker 参数来避免因为宕机丢消息**，简单说就是可靠性优先的场景都应该使用同步。Consumer保证消息成功消费的关键在于确认的时机，不要在收到消息后就立即发送消费确认，而是应该在执行完所有消费业务逻辑之后，再发送消费确认。

#### 顺序性！

在有些业务逻辑下，必须保证顺序，比如订单的生成、付款、发货，这个消息必须按顺序处理才行。简单来说，为了保证整个 **Topic全局消息有序**，**只能消除所有的并发处理**，各部分都设置成单线程处理 ，这时候就完全牺牲RocketMQ的高并发、高吞吐的特性了。**部分顺序消息**相对比较好实现，生产端需要做到把同 ID 的消息发送到同一个 Message Queue ；在消费过程中，要做到从同一个Message Queue读取的消息顺序处理——消费端不能并发处理顺序消息，这样才能达到部分有序。
...
![消息迁移扩容消费](https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/weixin-mianznxrocketmqessw-69aa8004-45e9-4e41-b628-1d7ed6d94c92.jpg)
...
![消息过滤](https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/weixin-mianznxrocketmqessw-f2c8bf50-dc51-44c8-9d71-b8a22af199c4.jpg)
