...
针对于编写好的．java源文件进行编译(使用javac.exe)，会生成一个或多个.cLass字节码文件。接着，我们使用 java.exe命令对指定的.cLass文件进行解释运行。这个解释运行的过程中，我们需要将.cLass字节码文件加载到内存中。加载到内存中的.class文件对应的结构即为CLass的一个实例。
...
```java
public  static void sort(int[] arr) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
//
    Class<Integer> aClass = Integer.class;
    Integer integer = aClass.newInstance();
}
```

要 求： 1）**类必须有一个无参数的构造器。** 否则报：InstantiationException 2）类的构造器的**访问权限需要足够。** 否则报：IllegalAccessException

---

> 抛出的异常也就是可能会发生的异常叫做编译时异常，运行时异常是比如说数组越界之类的，会需要提前设置程序才能通过。
...
（5）**取得指定对象obj上此Field的属性内容**，指定，也就是一个具体的实例，比如一个学生
