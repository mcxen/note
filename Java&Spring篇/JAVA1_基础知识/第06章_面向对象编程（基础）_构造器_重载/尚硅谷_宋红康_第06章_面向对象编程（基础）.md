...
- 使用情况
  ...
  在Java中，可以通过创建一个没有指定名称的匿名对象，来直接调用类的方法或者使用类的属性。匿名对象通常用于一次性使用的场景，它们以更简洁的方式编写代码，并且不需要在代码中额外定义对象引用变量。匿名对象通常在与对象的方法调用相关的行内创建。
  
  要创建匿名对象，请按以下方式操作：
  
  1. 按照普通对象的创建方式编写代码，例如：
  
  ```java
  MyObject obj = new MyObject(); // 在这里 MyObject 是类的名称
  ```
  
  2. 将分号`;`替换为花括号` { /* 类定义块 */ }`，这意味着不再创建一个新的对象引用变量，而是直接创建一个匿名对象，例如：
  
  ```java
  new MyObject() { /* 类定义块 */ }
  ```
  
  在第二步中，我们定义了一个MyObject类的匿名对象，并在其后面使用了花括号`{}`来定义匿名对象的类定义块或匿名对象的方法和属性。在类定义块中，**我们可以重写MyObject类中的方法、添加新的方法或定义新的属性。**
  
  例如，我们可以通过以下代码创建一个匿名对象，并在其中调用其方法：
  ...
  在上面的代码中，我们创建了一个匿名对象，并在其中编写了一个名为`myMethod()`的方法，该方法用于输出“Hello World”的字符串。然后，我们直接调用了匿名对象的该方法。
  
  总之，Java中的匿名对象可以让我们在不创建对象引用变量的情况下，使用对象的方法和属性。它们通常在使用Java I/O和Swing GUI等一些一次性处理的场景下使用最多。
  
  ---
  
  
  
  在Java的HashMap初始化时插入数据可以使用以下方式：
  ...
  这个方式使用了Java中的“双括号初始化”语法，其中第一对括号用于创建匿名类，第二对括号用于在匿名类中插入代码块。在代码块中，我们可以使用`put()`方法向HashMap中插入键值对。
  
  需要注意的是，在使用双括号初始化的时候会创建一个匿名子类，如果对内存有所要求可以选择使用正常的构造函数来初始化HashMap，然后用`put()`方法依次添加键值对。
  
  具体解释如下：
  
  1. 首先，使用`new`关键字创建了一个`HashMap`对象，右边的大括号相当于匿名内部类的一个类初始化块，用于初始化`HashMap`对象。
  
  2. 每个大括号内部，是另外一对花括号`{}`，这样的写法称为“双括号初始化”。使用双括号结构的代码会创建一个继承`HashMap`的匿名类，并在其中进行初始化。
  
  3. 大括号内部使用了Map的`put()`方法，它会把元素添加到Map中。
  
  4. 使用分号`;`将代码块结束。
  
  需要注意的是，使用双括号初始化语法虽然方便，但也有一些潜在的问题，尤其是会创建一个匿名子类对象，导致内存占用增加。所以在实际开发中还需要根据具体情况权衡使用。
