...
2. **实现/继承数量不同**：一个类只能继承一个抽象类，但可以实现多个接口。
3. **包含变量不同**：抽象类无要求，而接口只能包含常量。
4. **构造函数不同**：抽象类可以有构造函数，而接口不能有构造函数。
...
>    >
>    > ![image-20240229165747139](https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20240229165747139.png)
>
...
- 声明接口用interface，接口的成员声明有限制：
  ...
  - （5）私有方法（JDK9.0 及以上）
- 类可以实现接口，关键字是implements，而且支持多实现。**如果实现类不是抽象类，就必须实现接口中所有的抽象方法。**如果实现类既要继承父类又要实现父接口，那么继承（extends）在前，实现（implements）在后。（抽象方法和默认方法不同）
