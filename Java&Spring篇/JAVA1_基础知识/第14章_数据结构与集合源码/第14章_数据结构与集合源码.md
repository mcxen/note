...
<img src="https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20230408170956241.png" alt="image-20230408170956241" style="zoom:50%;" />

在调试的过程中，DEBUG可以切换view as object，就可以看到这个map对象的所有的元素的信息了

这里还可以右击之后**自定义显示模版（customize data views)**

<img src="https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20230408172300189.png" alt="image-20230408172300189" style="zoom: 50%;" />
...
<img src="https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20220521111904272.png" alt="image-20220521111904272" style="zoom:67%;" />
...
* 数组的初始化容量，如果在构建ArrayList与Vector的集合对象时，没有显式指定初始化容量，那么Vector的内部数组的初始容量默认为10，而**ArrayList在JDK 6.0 及之前的版本也是10**，**JDK8.0 之后的版本ArrayList初始化为长度为0的空数组，之后在添加第一个元素时，再创建长度为10的数组。**原因：
...
使用数组的目的是**查询和添加的效率高，**可以根据索引直接定位到某个table[index]。

jdk7时底层的数据结构是：数组+单向链表。 而jdk8时，**底层的数据结构是：数组+单向链表+红黑树。**

<img src="https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/image-20221029144811305.png" alt="image-20221029144811305" style="zoom:80%;" />

### 8.2 HashMap中数据添加过程

#### 8.2.1 JDK7中过程分析

JDK7中是HashMap里有一个Entry的类，这个类实现了Map的Entry接口
...
此哈希值1再经过某种运算(hash())，得到哈希值2。此哈希值2再经过某种运算(indexFor())，
确定在底层table数组中的索引位置i。
       ...
       判断key1的哈希值2与key2的哈希值是否相同：
         （3） 如果哈希值不同，则(key1,value1)直接添加成功   ------位置2
              如果哈希值相同，则需要继续调用key1所在类的equals()方法，将key2放入equals()形参进行判断
                （4） equals方法返回false : key1和key2具体的数值是不相等的则(key1,value1)直接添加到链表下成功   ------位置3
                      equals方法返回true :  key1和key2具体的数值相等，默认情况下，value1会覆盖value2。
...
<img src="https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20230408120511815.png" alt="image-20230408120511815" style="zoom:50%;" />
...
#### 8.2.2 JDK8相较于JDK7的不同之处：

① 使用HashMap()的构造器创建对象时，并没有在底层初始化长度为16的table数组。当首次添加(key,value)时，进行判断， 如果发现table尚未初始化，则对数组进行初始化。

② jdk8中添加的`key,value`封装到了`HashMap.Node`类的对象中。而非jdk7中的`HashMap.Entry`。

③ jdk8中新增的元素所在的索引位置如果有其他元素。在经过一系列判断后，如果能添加，则是**旧的元素指向新的元素**，就是尾部插入，尾插法，而非**jdk7中的新的元素指向旧的元素**，JDK7为头部插入，头插法。“七上八下”

④ jdk7时底层的**数据结构是：数组+单向链表。** 而jdk8时，底层的数据结构是：**数组+单向链表+红黑树。**

红黑树出现的时机：**当某个索引位置i上的链表的长度达到8，且数组的长度超过64时，此索引位置上的元素要从单向链表改为红黑树。**如果索引i位置是红黑树的结构，**当不断删除元素的情况下，当前索引i位置上的元素的个数低于6时，要从红黑树改为单向链表。**

改为红黑树就是为了**加快查找，加快插入。**红黑树进行`put()/get()/remove()` 操作的时间复杂度为`O(Logn)`，比单向链表的时间复杂度`O(n)`的好。性能更高。
...
key-value被封装为HashMap.Node类型或HashMap.TreeNode类型，它俩都**直接或间接的实现了Map.Entry接口。**

存储到table数组的可能是Node结点对象，也可能是TreeNode结点对象，它们也是Map.Entry接口的实现类。即table[index]下的映射关系可能串起来一个链表或一棵红黑树。

![image-20220514190904009](https://cdn.jsdelivr.net/gh/52chen/imagebed2023@main/picgo/image-20220514190904009-1661448231966.png)
...
![image-20230408172056711](https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20230408172056711.png)

这里的47 和 31 对应同一个hash值，所以构成了链表，47是后续加入的，所以链表尾部，是插入在后面，31指向47.
...
![image-20230408171750049](https://fastly.jsdelivr.net/gh/52chen/imagebed2023@main/uPic/image-20230408171750049.png)
...
```java
Node<K,V> newNode(int hash, K key, V value, Node<K,V> me) {
...
在 Java 中，ArrayList 的大小理论上没有固定限制，但它受限于以下两个因素：

​	1.	**最大索引值**：ArrayList 的最大容量是 Integer.MAX_VALUE - 8，也就是大约 2^31-1 个元素（约 21 亿）。这是因为 ArrayList 内部是基于数组实现的，数组的大小受限于 int 的索引范围。

​	2.	**内存限制**：虽然 ArrayList 能支持非常大的容量，但实际能存储的元素数量取决于你程序的可用内存。如果没有足够的内存来存放这么大的数组，程序会抛出 OutOfMemoryError。



因此，虽然没有硬性定义的大小限制，ArrayList 的大小主要依赖于内存和 JVM 限制。