...
- [导入表的问题](#导入表的问题)
- [第三章_最基本的SELECT语句](#第三章_最基本的select语句)
  - [1. SQL语言的规则和规范](#1-sql语言的规则和规范)
    - [1) 基本规则](#1-基本规则)
    - [2) SQL大小写规范（建议遵守）](#2-sql大小写规范建议遵守)
    - [3) 注释](#3-注释)
    - [4) 命名规则](#4-命名规则)
  - [2. 基本的SELECT语句](#2-基本的select语句)
...
- [第四章_运算符](#第四章_运算符)
  ...
  - [2. 比较运算符](#2-比较运算符)
    ...
    - [5) 最小值运算符](#5-最小值运算符)
    - [6) 最大值运算符](#6-最大值运算符)
    - [7) BETWEEN AND运算符](#7-between-and运算符)
    - [8) IN运算符](#8-in运算符)
    - [9) NOT IN运算符](#9-not-in运算符)
    - [10) LIKE运算符](#10-like运算符)
    - [11) ESCAPE](#11-escape)
    - [12) REGEXP运算符](#12-regexp运算符)
  - [3. 逻辑运算符](#3-逻辑运算符)
  - [4. 位运算](#4-位运算)
  - [5. 运算符的优先级](#5-运算符的优先级)
  - [扩展：使用正则表达式查询](#扩展使用正则表达式查询)
- [第五章_排序与分页](#第五章_排序与分页)
  - [1. 排序规则](#1-排序规则)
    - [1) 单列排序](#1-单列排序)
    - [2) 多列排序](#2-多列排序)
  - [2. 分页](#2-分页)
- [第六章_多表查询](#第六章_多表查询)
  - [1. 多表查询分类讲解](#1-多表查询分类讲解)
    - [1) 自连接](#1-自连接)
    - [2) 内连接与外连接](#2-内连接与外连接)
  - [2. UNION的使用](#2-union的使用)
  - [3.七种SQL JOINS的实现](#3七种sql-joins的实现)
  - [4. SQL99语法的新特性](#4-sql99语法的新特性)
    - [1) 自然连接](#1-自然连接)
    - [2) USING连接](#2-using连接)
  - [5. 小结](#5-小结)
- [第七章_单行函数](#第七章_单行函数)
  ...
  - [3. 日期和时间函数](#3-日期和时间函数)
    - [1) 获取日期、时间](#1-获取日期时间)
    - [2) 日期与时间戳的转换](#2-日期与时间戳的转换)
    - [3) 获取月份、星期、星期数、天数等函数](#3-获取月份星期星期数天数等函数)
    - [4) 日期的操作函数](#4-日期的操作函数)
    - [5) 时间和秒钟转换的函数](#5-时间和秒钟转换的函数)
    - [6) 计算日期和时间的函数](#6-计算日期和时间的函数)
    - [7)  日期的格式化与解析](#7--日期的格式化与解析)
  - [4. 流程控制函数](#4-流程控制函数)
  - [5. 加密与解密函数](#5-加密与解密函数)
  - [6. MySQL信息函数](#6-mysql信息函数)
- [第八章_聚合函数](#第八章_聚合函数)
  - [1. 聚合函数介绍](#1-聚合函数介绍)
    - [1) AVG和SUM函数](#1-avg和sum函数)
    - [2) MIN和MAX函数](#2-min和max函数)
    - [3) COUNT函数](#3-count函数)
  - [2. GROUP BY](#2-group-by)
    - [1) 基本使用](#1-基本使用)
    - [2) 使用WITH ROLLUP](#2-使用with-rollup)
  - [3. HAVING](#3-having)
    - [1) 基本使用](#1-基本使用-1)
    - [2) WHERE和HAVING的对比](#2-where和having的对比)
  - [4. SELECT的执行过程](#4-select的执行过程)
    - [1) 查询的结构](#1-查询的结构)
    - [2) SQL的执行原理](#2-sql的执行原理)
- [第九章_子查询](#第九章_子查询)
  - [1. 基本使用](#1-基本使用-2)
  - [2. 子查询的分类](#2-子查询的分类)
  - [3. 单行子查询](#3-单行子查询)
    - [1) 单行比较操作符](#1-单行比较操作符)
    - [2) 代码示例](#2-代码示例)
    - [3) CASE中的子查询](#3-case中的子查询)
    - [4) 子查询中的空值问题](#4-子查询中的空值问题)
    - [5) 非法使用子查询](#5-非法使用子查询)
  - [4. 多行子查询](#4-多行子查询)
    - [1) 多行比较操作符](#1-多行比较操作符)
    - [2) 代码示例](#2-代码示例-1)
  ...
  - [5. 相关子查询](#5-相关子查询)
    - [1) 代码示例](#1-代码示例)
    - [2) EXISTS 与 NOT EXISTS 关键字](#2-exists-与-not-exists-关键字)
    - [3) 相关更新](#3-相关更新)
    - [4) 相关删除](#4-相关删除)
  - [6. 思考题](#6-思考题)
  - [7. 课后练习](#7-课后练习)
- [第十章_创建和管理表  DDL](#第十章_创建和管理表--ddl)
  - [1. 基础知识](#1-基础知识)
    - [1) 标识符命名规则](#1-标识符命名规则)
    - [2) MySQL中的数据类型](#2-mysql中的数据类型)
  ...
  - [6. 清空表](#6-清空表)
  - [7. 内容扩展](#7-内容扩展)
    - [拓展1：阿里巴巴《Java开发手册》之MySQL字段命名](#拓展1阿里巴巴java开发手册之mysql字段命名)
    - [扩展2：操作注意要求](#扩展2操作注意要求)
    - [扩展3：MySQL8新特性—DDL的原子化](#扩展3mysql8新特性ddl的原子化)
- [第11章_数据处理之增删改](#第11章_数据处理之增删改)
  - [1. 插入数据](#1-插入数据)
    - [1) 方式1：VALUES的方式添加](#1-方式1values的方式添加)
    - [2) 方式2：将查询结果插入到表中](#2-方式2将查询结果插入到表中)
  - [2. 更新数据](#2-更新数据)
  - [3. 删除数据](#3-删除数据)
  - [4. MySQL8新特性：计算列](#4-mysql8新特性计算列)
- [第12章_MySQL数据类型精讲](#第12章_mysql数据类型精讲)
  - [1. MySQL中的数据类型](#1-mysql中的数据类型)
  - [2. 整数类型](#2-整数类型)
    - [1) 类型介绍](#1-类型介绍)
    - [2) 可选属性](#2-可选属性)
    - [3) 适用场景](#3-适用场景)
...
- [第13章_约束](#第13章_约束)
  ...
  - [5. 自增列：AUTO_INCREMENT](#5-自增列auto_increment)
    ...
    - [2) 关键字](#2-关键字-3)
    - [3) 特点](#3-特点-3)
    - [4) 如何指定自增约束](#4-如何指定自增约束)
    - [5) 删除自增约束](#5-删除自增约束)
    - [6) MySQL 8.0新特性—自增变量的持久化](#6-mysql-80新特性自增变量的持久化)
  - [6. FOREIGN KEY 约束](#6-foreign-key-约束)
    - [1) 作用](#1-作用-4)
    - [2) 关键字](#2-关键字-4)
    - [3) 主表和从表/父表和子表](#3-主表和从表父表和子表)
    - [4) 特点](#4-特点)
    - [5) 添加外键约束](#5-添加外键约束)
    - [6) 约束等级](#6-约束等级)
    - [7) 删除外键约束](#7-删除外键约束)
...
- [第14章_视图](#第14章_视图)
  ...
  - [3. 创建视图](#3-创建视图)
    - [1) 创建单表视图](#1-创建单表视图)
    - [2) 创建多表联合视图](#2-创建多表联合视图)
    - [3) 基于视图创建视图](#3-基于视图创建视图)
  - [4. 查看视图](#4-查看视图)
  - [5. 更新视图的数据](#5-更新视图的数据)
    - [1) 一般情况](#1-一般情况)
    - [2) 不可更新的视图](#2-不可更新的视图)
...
- [第15章_存储过程与函数](#第15章_存储过程与函数)
  ...
  - [6. 关于存储过程使用的争议](#6-关于存储过程使用的争议)
    ...
    - [2) 缺点](#2-缺点)
    - [3) 小结](#3-小结)
- [第16章_变量、流程控制与游标](#第16章_变量流程控制与游标)
  - [1. 变量](#1-变量)
    - [1) 系统变量](#1-系统变量)
    - [2) 用户变量](#2-用户变量)
  - [2. 定义条件与处理程序](#2-定义条件与处理程序)
    ...
    - [4) 案例解决](#4-案例解决)
  - [3. 流程控制](#3-流程控制)
    - [1) 分支结构之 IF](#1-分支结构之-if)
    - [2) 分支结构之 CASE](#2-分支结构之-case)
    - [3) 循环结构之LOOP](#3-循环结构之loop)
    - [4) 循环结构之WHILE](#4-循环结构之while)
    - [5) 循环结构之REPEAT](#5-循环结构之repeat)
    - [6) 跳转语句之LEAVE语句](#6-跳转语句之leave语句)
    - [7) 跳转语句之ITERATE语句](#7-跳转语句之iterate语句)
  - [4. 游标](#4-游标)
    - [1)  什么是游标（或光标）](#1--什么是游标或光标)
    - [2) 使用游标步骤](#2-使用游标步骤)
    - [3) 举例](#3-举例)
    - [4) 小结](#4-小结)
  - [补充：MySQL 8.0的新特性—全局变量的持久化](#补充mysql-80的新特性全局变量的持久化)
- [第17章_触发器](#第17章_触发器)
  - [1. 触发器概述](#1-触发器概述)
  - [2. 触发器的创建](#2-触发器的创建)
    - [1) 语法](#1-语法)
  ...
  - [4. 触发器的优缺点](#4-触发器的优缺点)
    ...
    - [2) 缺点](#2-缺点-1)
    - [3) 注意点](#3-注意点)
- [第18章_MySQL8其他新特性](#第18章_mysql8其他新特性)
  - [1. MySQL8新特性概述](#1-mysql8新特性概述)
    - [1) MySQL8.0 新增特性](#1-mysql80-新增特性)
    - [2) MySQL8.0 移除的旧特性](#2-mysql80-移除的旧特性)
  - [2. 新特性1：窗口函数](#2-新特性1窗口函数)
    - [1) 使用窗口函数前后对比](#1-使用窗口函数前后对比)
    - [2) 窗口函数分类](#2-窗口函数分类)
    - [3) 语法结构](#3-语法结构)
    - [4) 分类讲解](#4-分类讲解)
      - [1) 序号函数](#1-序号函数)
      ...
      - [5) 其他函数](#5-其他函数)
    - [5) 小结](#5-小结-1)
  - [3. 新特性2：公用表表达式](#3-新特性2公用表表达式)
    - [1) 普通公用表表达式](#1-普通公用表表达式)
    - [2)  递归公用表表达式](#2--递归公用表表达式)
    - [3) 小结](#3-小结-1)
...
```mysql
SELECT last_name, department_name
FROM employees emp FULL OUTER JOIN department dep
ON emp.`department_id` = dep.`department_id`;
```
...
```mysql
SELECT salary ,last_name ,CASE WHEN   salary > 10000 THEN '白骨精' END '评级' FROM employees w;
